    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={{ width: '100%', alignItems: 'center', marginTop: 20, backgroundColor: 'transparent', height: height * 0.55 }}>
                <FlatList
                    data={data}
                    keyExtractor={(item) => item.id_aktivitas}
                    ...
                    renderItem={({ item, index }) => {
                        return (
                            <TouchableOpacity onLongPress={() => hapus(item.id_aktivitas)} key={index} style={{ width: '93%', height: 70, backgroundColor: '#fff', borderRadius: 5, flexDirection: 'row', marginTop: 10 }}>
                                <View style={{ width: '70%', height: '100%', backgroundColor: 'transparent', padding: 10, justifyContent: 'space-between' }}>
                                    <Text style={{ fontSize: 18, fontWeight: 'bold', color: '#0e918c' }}>{item.nama_aktivitas}</Text>
